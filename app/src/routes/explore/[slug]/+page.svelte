...
<div class="mx-auto mt-8 max-w-6xl px-4 sm:px-6 lg:px-8">
	<div class="relative flex flex-col items-center space-y-6 text-center">
		<h1 class="text-2xl font-semibold uppercase italic">{data.category.title}</h1>
		<h3 class=" max-w-xl text-lg">
			{data.category.description}
		</h3>
		<div class="h-4 w-full border-t-4 border-primary"></div>
	</div>
</div>

<div
	class="divide-y divide-gray-200 rounded-lg bg-gray-200 shadow-sm sm:grid sm:grid-cols-2 sm:gap-px sm:divide-y-0 lg:h-[60vh]"
>
	<a
		href="/article/{data.articles[0].slug}"
		class="group relative overflow-hidden bg-white focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500 sm:rounded-tr-lg lg:row-span-2"
	>
		<img
	...
	<a
		...
		class="group relative overflow-hidden bg-white focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500 lg:row-span-1"
	>
		<img
